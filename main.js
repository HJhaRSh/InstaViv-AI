...
if (robotAvatar) {
  // --- Voice Synthesis & HUD ---
  const sentences = [
    "System initialized. Welcome to InstaViv A I.",
    "We are building the next generation of A I creators.",
    "Deploying experiential learning models."
  ];

  let isSpeaking = false;
  let currentTypeInterval = null;
  const hud = document.getElementById('avatar-speech-hud');
  const speechWrapper = document.getElementById('avatar-speech-wrapper');
  const initWrapper = document.getElementById('init-ai-wrapper');
  const initBtn = document.getElementById('init-ai-btn');
  const chatWrapper = document.getElementById('avatar-chat-wrapper');
  ...
  // Pre-load voices if necessary (some browsers load asynchronously)
  ...
  if (initBtn) {
    initBtn.addEventListener('click', () => {
      ...
      const text = sentences[Math.floor(Math.random() * sentences.length)] + " Hi, how can I help you today?";
      
      // 1. Text-to-Speech API
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.pitch = 0.6; // Slightly deep/robotic
      utterance.rate = 1.0;
      
      const voices = window.speechSynthesis.getVoices();
      // Try to pick a techy/robotic voice if available
      const techVoice = voices.find(v => v.name.includes('Google') || v.name.includes('Samantha') || v.name.includes('David')) || voices[0];
      ...
      if (speechWrapper) speechWrapper.style.opacity = '1';
      hud.innerHTML = '';
      const initNode = document.createElement('div');
      initNode.className = 'chat-msg ai-msg';
      hud.appendChild(initNode);
      if (!isMuted) robotAvatar.classList.add('is-speaking');
      ...
      currentTypeInterval = setInterval(() => {
        if (i < text.length) {
          initNode.textContent += text.charAt(i);
          i++;
          ...
          if (isMuted) {
            isSpeaking = false;
            robotAvatar.classList.remove('is-speaking');
      ...
      // 4. Reset after speech ends
      utterance.onend = () => {
        isSpeaking = false;
        robotAvatar.classList.remove('is-speaking');
        if (chatStop) chatStop.style.display = 'none';
        if (chatSend) chatSend.style.display = 'block';
        if (chatWrapper) {
          chatWrapper.style.opacity = '1';
          chatWrapper.style.pointerEvents = 'auto';
        }
      ...
      if (!isMuted) {
        if (chatStop) chatStop.style.display = 'block';
  ...
  if (chatMute) {
    chatMute.addEventListener('click', () => {
      isMuted = !isMuted;
      if (isMuted) {
        chatMute.style.background = 'var(--warm-coral)';
        chatMute.style.borderColor = 'var(--warm-coral)';
        chatMute.style.color = '#000';
        chatMute.textContent = 'UNMUTE';
        if (isSpeaking) {
          window.speechSynthesis.pause();
          robotAvatar.classList.remove('is-speaking');
        }
      } else {
        chatMute.style.background = 'transparent';
        chatMute.style.borderColor = 'var(--electric-cyan)';
        chatMute.style.color = 'var(--electric-cyan)';
        chatMute.textContent = 'MUTE';
        if (isSpeaking) {
          window.speechSynthesis.resume();
          robotAvatar.classList.add('is-speaking');
        }
      }
    });
  }
  ...
  if (chatSend) {
    chatSend.addEventListener('click', async () => {
      ...
      userNode.className = 'chat-msg user-msg';
      userNode.textContent = `You: ${message}`;
      
      const aiNode = document.createElement('div');
      aiNode.className = 'chat-msg ai-msg';
      aiNode.textContent = `AI: ...`;
      
      hud.appendChild(userNode);
      hud.appendChild(aiNode);
      hud.scrollTop = hud.scrollHeight;
      ...
      try {
        ...
        currentTypeInterval = setInterval(() => {
          if (i < aiText.length) {
            aiNode.textContent += aiText.charAt(i);
            i++;
            hud.scrollTop = hud.scrollHeight;
          } else {
            clearInterval(currentTypeInterval);
        ...
        aiNode.style.color = 'var(--warm-coral)';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dynamicText) {
    const words = ["Next Generation.", "Future.", "Curriculum.", "Classrooms."];
    let wordIndex = 0;
